...
``_open``      fixed    +1    open-curve shape factories (``line`` / ``arc``)
...
binds them onto the class below, so callers write ``LineMesh.circle(...)`` /
...
for _name, _fn in {**_CLOSED_FACTORIES, **_OPEN_FACTORIES,
                   **_ASSEMBLE_FACTORIES, **_MORPH_FACTORIES}.items():
