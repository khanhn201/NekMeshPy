...
def _shell(order, n_face=2, n_radial=2):
    ...
    cube = quadmesh.surface.box(3.0, n_face, order=order)
...
def test_quad_round_trip_through_entity_store(order):
    ...
    qm = quadmesh.region.rectangle([[0, 0, 0], [4, 0, 0], [4, 3, 0], [0, 3, 0]],
                            3, 2, order=order)
...
def test_shared_edge_resolves_to_same_nodes(order):
    ...
    nx, ny = 2, 1
    qm = quadmesh.region.rectangle([[0, 0, 0], [2, 0, 0], [2, 1, 0], [0, 1, 0]],
                            nx, ny, order=order)
    assert qm.n_quads == 2
    _, conn = conformal(qm)
...
def test_non_conforming_edge_nodes_rejected(order):
    ...
    qm = quadmesh.region.rectangle([[0, 0, 0], [2, 0, 0], [2, 2, 0], [0, 2, 0]],
                            2, 2, order=order)
    local = conform.gather_edge_nodes(qm.edge_nodes, qm.quad, qm.flip)
...
def test_order1_conformal_is_points_and_conn():
    qm = quadmesh.region.rectangle([[0, 0, 0], [2, 0, 0], [2, 2, 0], [0, 2, 0]], 2, 2)
    nodes, conn = conformal(qm)
...
def test_quadmesh_brep_storage(order):
    ...
    src = quadmesh.region.rectangle([[0, 0, 0], [2, 0, 0], [2, 1, 0], [0, 1, 0]],
                             2, 1, order=order)
