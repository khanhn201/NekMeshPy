...
def _disc(n, order=1, center_scale=CS, radial=RADIAL, wall_tag="wall"):
    ...
    fr = quadmesh.region.quadrant_seam_fractions(n, radial, center_scale)
    ang = [0.0, np.pi / 2, np.pi, 3 * np.pi / 2, 2 * np.pi]
    seams = [_radius(a, fr, order) for a in ang[:4]]
    seams.append(seams[0])
    return [quadmesh.region.quadrant_ogrid(
...
def test_a_bowed_seam_is_meshed_exactly(order):
    ...
    q = quadmesh.region.quadrant_ogrid(arc, s1, s2, RADIAL, center_scale=CS)
...
def test_tags_ride_up_from_the_line_level():
    ...
    q2 = quadmesh.region.quadrant_ogrid(arc, s1, s2, RADIAL, center_scale=CS,
...
def test_input_contract_is_loud():
    ...
    with pytest.raises(ValueError, match="seam1/seam2"):
        quadmesh.region.quadrant_ogrid(arc, s1, s2, RADIAL, side_tags={"bottom": "x"})
    with pytest.raises(ValueError, match="center_scale"):
        quadmesh.region.quadrant_ogrid(arc, s1, s2, RADIAL, center_scale=0.0)
...
def test_quadrant_core_is_the_factory_s_own_core(n):
    ...
    s1, s2 = _radius(0.0, fr), _radius(np.pi / 2, fr)
    arc = linemesh.shape.arc(R, 2 * n, start_theta=0.0, end_theta=np.pi / 2)
    core = quadmesh.region.quadrant_core(arc, s1, s2, center_scale=CS)
...
def test_quadrant_core_rejects_bad_shapes():
    fr = quadmesh.region.quadrant_seam_fractions(2, RADIAL, CS)
    s1, s2 = _radius(0.0, fr), _radius(np.pi / 2, fr)
    even = linemesh.shape.arc(R, 3, start_theta=0.0, end_theta=np.pi / 2)
    with pytest.raises(ValueError, match="2\\*n\\+1 points"):
        quadmesh.region.quadrant_core(even, s1, s2)
    arc = linemesh.shape.arc(R, 4, start_theta=0.0, end_theta=np.pi / 2)
    with pytest.raises(ValueError, match="center_scale"):
        quadmesh.region.quadrant_core(arc, s1, s2, center_scale=1.0)
    with pytest.raises(ValueError, match="at least 3 points"):
        quadmesh.region.quadrant_core(arc, _radius(0.0, np.array([0.0, 0.5])), s2)
