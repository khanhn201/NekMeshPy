...
Everything here works on a tet mesh of the *interior*: the P1 Laplacian, the three seam
fields, the sign-based partition into legs, and marching-tets isosurface extraction.
The point of going volumetric is that a harmonic field with a no-flux wall has grad(u)
tangent to the wall, so every level set cuts the wall at a right angle -- which a field
solved on the wall alone cannot tell you anything about.
"""
import numpy as np
...
# -- tet mesh ----------------------------------------------------------------
def cap_surface(V, F, loops):
    """Close an open surface by fan-triangulating each boundary loop from its centroid.
    Returns ``(V, F, cap_node_sets)``; the wall vertices keep their indices."""
    caps, faces, n0 = [], [F], V.shape[0]
    sets = []
    for ring in loops:
        ci = n0 + len(caps)
    ...
    return np.vstack([V, np.array(caps)]), np.vstack(faces), sets


def tet_mesh(V, F, near, far, ramp, centre):
    """Tet-mesh the closed surface ``(V, F)`` with gmsh, keeping its nodes in place.

    Element size is **graded** by a background field: ``near`` at the junction, growing
    to ``far`` over ``ramp``.
    ...
    could remesh it does not work on this geometry: the wall is a smooth closed tube with
    no sharp edges to split on, so it comes out either as one unparametrizable patch or,
    at an angle small enough to succeed, as hundreds of them, every boundary of which
    would then be forced into the mesh as a feature curve.

    The three ``MeshSizeFrom*`` switches are turned off so the field is the *only* thing
    setting size; otherwise the surface mesh's own spacing is extended into the volume
    and quietly overrides it."""
    import gmsh
    gmsh.initialize()
    try:
    ...
    if d.max() > 1e-9 or np.unique(row).size != nv:
        raise RuntimeError("gmsh moved the input surface nodes")
    ...
    tag_row = np.zeros(int(nt.max()) + 1, dtype=np.int64)
...
def cap_nodes(P, TET, loops, tol=1e-6):
    """Which boundary nodes lie on each opening.

    Remeshing throws away the numbering the caps used to be identified by, so find them
    geometrically instead.  Each opening is planar and nothing else in the domain lies in
    its plane, which makes the test both simple and unambiguous."""
    f = np.sort(np.concatenate([TET[:, [0, 1, 2]], TET[:, [0, 1, 3]],
                                TET[:, [0, 2, 3]], TET[:, [1, 2, 3]]]), axis=1)
    uf, cnt = np.unique(f, axis=0, return_counts=True)
    bnd = np.unique(uf[cnt == 1])
    out = []
    for R in loops:
...
class FieldWalker:
    ...
    def __init__(self, P, TET, u):
        ...
        self.grad, _ = tet_gradients(P, TET)
...
def tet_laplacian(P, TET):
    """P1 stiffness ``K_ij = vol * grad(lam_i) . grad(lam_j)`` -- the tet sibling of the
    cotangent Laplacian the surface solver uses."""
    g, vol = tet_gradients(P, TET)
...
def solve_dirichlet(L, n, nodes, vals):
    """Laplace with Dirichlet ``vals`` at ``nodes``; every other boundary is no-flux,
    which is the natural condition and so needs nothing said about it."""
    u = np.zeros(n)
    u[nodes] = vals
...
def seam_fields(L, n, caps):
    ...
    Exactly ``carotid.py``'s ``seam_fields``, with the solve moved into the volume."""
    ...
    for k in range(3):
        ...
        u = solve_dirichlet(L, n, nodes, vals)
        U[:, k] = u - u[caps[k]].mean()
    return U
...
def leg_label_tets(U, TET):
    """Which leg each tet belongs to (1/2/3), from the sign pattern of the three fields
    at its centroid -- the volumetric form of ``carotid.py``'s ``leg_label``."""
    c = U[TET].mean(axis=1)
    a, b, d = c[:, 0], c[:, 1], c[:, 2]
    lab = np.zeros(TET.shape[0], dtype=np.int64)
    lab[(b > 0) & (d < 0)] = 1
    lab[(a < 0) & (d > 0)] = 2
    lab[(a > 0) & (b < 0)] = 3
    return lab
