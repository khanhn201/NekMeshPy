...
| `flow_past_half_cylinder.py` | `quadmesh.shape.structured` over four tagged edges, the bottom one a welded semicircular bump → span-`loft` (caps `front`/`back`) |
...
| `high_order_quad.py` | `quadmesh.shape.sphere(order=ORDER)` — all `(N+1)²` surface nodes on the true sphere; `.vtu` `VTK_LAGRANGE_QUADRILATERAL` |
...
The 2-D section meshers (`quadmesh.shape.ogrid` / `structured` / `half_ogrid` /
...
factory (`linemesh.shape.circle`, `quadmesh.shape.sphere`/`box`, `hexmesh.lift.annulus`, …) and each
