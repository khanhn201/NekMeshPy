...
Both are listed in `LIBRARY_ONLY` in `tests/test_examples.py`, which is what
exempts them from the "must define a `mesh` global" check.

| file | what it holds |
|---|---|
| `tjunction_lib.py` | `build_tjunction(...)`, the quadrant T-junction generalized into a function so it can be called at different radii and positions. Returns the merged crotch/transition core plus the three plain cross-sections at the legs' outward ends, for a caller to continue building from rather than re-derive. Used by `chimera_full.py` for both its T1 and T2 families |
| `coil_lib.py` | the serpentine coil's move table and tube radius. `serpentine_pipe.py` builds this coil standing alone and `chimera_full.py` sweeps a copy between each pair of T2 branches — one physical part, so one table. The shape is traced from a reference photo and is only ever placed, never rescaled |
