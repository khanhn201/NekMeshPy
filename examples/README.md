...
PYTHONPATH=. python examples/chimera.py                # two parallel pipes joined by N hairpin connectors
PYTHONPATH=. python examples/chimera_chain.py          # N_COPIES chimera units chained, alternating connector pipe
PYTHONPATH=. python examples/chimera_full.py           # chimera_chain's two ports fed by one serpentine coil
...
|---|---|
| `bifurcation.py` | vessel surface pipeline: seam fields → cut into legs → O-grid legs (`quadmesh.region.spined_ogrid`) → `loft`/`merge` → smooth (uses `data/car.{vtx,tri}`) |
| `circular_pipe.py` | `quadmesh.region.ogrid` disc extruded along an axis (`hexmesh.lift.extrude`) |
| `circular_pipe_tjunction.py` | analytic three-leg junction: shared seam arcs + spine → spined-O-grid legs (`quadmesh.region.spined_ogrid`) → `loft`/`merge` (no input geometry; the STL wall polish is present but off at `ORDER = 4` — see `SMOOTH_ITERS`) |
| `quadrant_pipe_tjunction.py` | welded small-branch T-junction, four `quadmesh.region.quadrant_ogrid` blocks per section. **One quadrant of the main pipe *is* a quadrant of the branch**: four regions (two legs, branch stub, two crotch caps) meet at the axes-crossing point `O`, and every interface between them is a quadrant face radiating from it — the branch's footprint disc contributes its `+z` and `-z` quadrants to the two legs' composite junction faces and its two lateral quadrants to the caps, while the legs share the wide `bypass` quadrant with each other. Each crotch is filled by `hexmesh.shape.tetra`: a quadrant face is itself a three-patch triangle (core + the two halves of its ring band), so handing three of them plus a wall patch to the generic tetrahedron split yields the octant of a 3-D O-grid — core cube + three radial slabs — with the block split the faces already carry. Exact at any order: every wall curve is carried as a parametrization and meshed with `loft_fn`, each leg's transition is a `hexmesh.assemble.loft_fn` (a plain `loft` is straight along the sweep), and the caps are nested `loft_fn` blocks evaluated at every node — so all wall nodes, high-order ones included, sit on the main or branch cylinder to `2.2e-16` at `ORDER` 1–4. Single watertight, conformal component |
| `chimera.py` | two parallel manifolds joined by `N_BR` hairpin tube connectors: `N_BR` copies of a T-junction are chained along pipe A exactly as before (`hexmesh.morph.translate` per junction, tolerance-welded leg ends), each branch stub extended with a short straight arm; pipe B is not authored at all — it is `pipe_a.rotate(np.pi, axis=(0, 0, 1), center=...)`, a rigid rotation (unlike a mirror, which inverts every element's Jacobian) offset purely along `x`, branches facing away from pipe A's; a `hexmesh.lift.sweep` bend per junction carries the arm's end along a declarative turtle-walk path (same pattern as `serpentine_pipe.py`) — a 180 degree U-turn, a straight run back toward `-x`, and a second U-turn restoring the original heading — with the straight run's length solved so the far end lands exactly on pipe B's own (rotated) arm end and welds by tolerance with no separately built return arm. `TOPOLOGY` toggles each junction between the quadrant-block decomposition (`"quadrant"`, default) and a two-saddle-point `quadmesh.region.spined_ogrid` half-ogrid junction (`"half_ogrid"`) whose seam can be rotated off-centre with `SEAM_OFFSET` |
| `chimera_chain.py` | `N_COPIES` copies of `chimera.py`'s unit chained along `z`, flush end to end (`L_HALF` translation per copy); the pipe that carries the connection between each pair of neighbours alternates A/B down the chain. The connector reaches the joint plane and welds away (`""`); the other pipe stops `GAP` short of it and is capped `wall` right there, so there is genuine empty space — no elements at all — between a dead-end and its neighbour, not just a differently-tagged flush cap. Only the chain's two true ends get `inlet`/`outlet`. The junction chain is built once with both outermost ends left bare, and pipe B still comes for free as `pipe_a.rotate(...)`; the four end stubs past the outermost junctions (one per pipe, per end, each its own length) are built independently — pipe B's in pipe A's own local frame and carried over by the same rotation — so nothing is derived twice |
| `chimera_full.py` | the whole manifold around `chimera_chain.py`: each of its two ports is reached by a riser -> T1 junction -> elbow, and each T1's `-y` branch carries a **chain of `N_T2` T2 junctions** hanging off one another's `-y` legs at `T2_SPACING` intervals (only the last capped). The two chains mirror, so their k-th junctions face each other across their own copy of the serpentine coil — `N_T2` parallel coils stacked down `-y`, each planar in its own x-z plane, each an 8-pass photo-traced `COIL_MOVES` turtle walk (shared verbatim with `serpentine_pipe.py`), placed but never rescaled. Negative-x T1 feeds chimera's *outlet*, positive-x its *inlet*, and the ports sit exactly 10 above the coil in `y`. Its real subject is **exact seams between differently-constructed pieces**, which `order > 1` forces (`hexmesh.assemble.merge` checks shared high-order nodes against `conform.entity_tol`, ~1e-9 x extent -- far tighter than a coordinate weld): `_reindex_geometry` re-expresses one section through another's index labels (a permutation, exact by construction, where a coordinate rotation is only approximate); `pattern_adapter` blends a *small* pattern gap (~0.03, T1's leg vs chimera's) with both end slices exact; and `weld_bridge` spans a *large* one (~0.94 median, T1's arc-length-stationed branch vs T2's uniform-angle leg) as a single `hexmesh.assemble.loft` -- rigid stubs off each side plus a blend across the gap, so there is no internal merge left to fail. Where a seam can be *removed* rather than made exact it is: the inbound connector and the coil sweep as one turtle walk, because `_weld` fuses by rounding to a `tol`-sized bucket and a ~1 ULP disagreement across that seam failed to weld wherever a coordinate landed exactly on a bucket edge. `FAST` swaps the 350k-element real chain for two capped stubs carrying its exact port pattern |
| `serpentine_pipe.py` | one `quadmesh.region.ogrid` disc swept along an analytic serpentine centerline (`hexmesh.lift.sweep`, `orientation="fixed"` against the coil's plane normal) — 8 passes + 7 U-bends + a hook at each end, from a turtle-walk move table of straights and arcs. Not top-bottom symmetric: passes 4/5 carry an extra `RAISE`, lifting the wide middle bridge (`U_R_MID`) that joins the two half-coils above the flanking hairpins (`U_R`). **This is `chimera_full.py`'s own coil** — same move table, same numbers — so the two build the same physical part, standalone here and swept between two T2 branches there. `linemesh.shape.sweep_fractions` lands a sweep station exactly on every straight↔arc junction, where the curvature jumps; `TARGET_LEN` is floored by the tightest U-turn rather than by element aspect, since `sweep_fractions` rounds to the *nearest* station count and a target near a turn's own arc length collapses it to one degenerate 180° hex |
| `rectangular_pipe.py` | `quadmesh.region.structured` duct extruded along an axis (`hexmesh.lift.extrude`) |
| `transfinite_block.py` | eight corners → trilinear grid → `hexmesh.lift.from_grid` |
| `backward_facing_step.py` | three `quadmesh.region.rectangle(side_tags={…})` grids → `merge` → span-`loft` (caps `front`/`back`) |
| `flow_past_cylinder.py` | `quadmesh.lift.annulus` (circle body → **named** square far-field loop) → span-`extrude` (body `cylinder`; sides `inlet`/`outlet`/`top`/`bottom` tagged on the outer loop; caps `front`/`back`) |
| `flow_past_plate.py` | `quadmesh.lift.annulus` around a thin ellipse → span-`extrude` (body `plate`; far-field sides tagged on the outer loop; caps `front`/`back`) |
| `flow_past_half_cylinder.py` | `quadmesh.region.structured` over four tagged edges, the bottom one a welded semicircular bump → span-`loft` (caps `front`/`back`) |
| `flow_past_sphere.py` | `hexmesh.lift.annulus` between a closed sphere surface and a closed cube surface (six `quadmesh.lift.from_grid` patches → `quadmesh.assemble.merge`, per-patch `element_tag`) — wall faces tagged from the surfaces' per-quad `element_tags` (body → `sphere`; far field → `inlet`/`outlet`/…) |
| `flow_past_hemisphere.py` | five-patch half cubed-sphere on the ground, each `half_box(patch_tags=…)` → `merge` (body → `hemisphere`) |
| `high_order_curve.py` | `linemesh.shape.circle(order=ORDER)` — every arc node placed on the true circle; `.vtu` `VTK_LAGRANGE_CURVE` |
| `high_order_quad.py` | `quadmesh.surface.sphere(order=ORDER)` — all `(N+1)²` surface nodes on the true sphere; `.vtu` `VTK_LAGRANGE_QUADRILATERAL` |
| `high_order_hex.py` | `hexmesh.lift.annulus(sphere, cube)` shell over two `order=ORDER` surfaces — curved inner wall on the true sphere; `.re2` stays linear, `.vtu` `VTK_LAGRANGE_HEXAHEDRON` |

The 2-D section meshers (`quadmesh.region.ogrid` / `structured` / `half_ogrid` /
...
(`linemesh.assemble.loft([…], element_tags=[…], loop=True)`), which is copied onto section edges — how
`flow_past_cylinder.py` splits its far field into `inlet`/`outlet`/`top`/`bottom`
via `linemesh.shape.rectangle(w, h, N, side_tags={"left": "inlet", …})` — the side/face tag
...
factory (`linemesh.shape.circle`, `quadmesh.surface.sphere`/`box`, `hexmesh.lift.annulus`, …) and each
...
factory fed a bare point array (`linemesh.assemble.loft`, `from_grid`) has only those
...
`linemesh.assemble.loft_fn`. The seam ring can't be refit per leg — all three legs share it — so
...
carried as their **surface parametrization** and meshed with `linemesh.assemble.loft_fn`
(sampling them into arrays and calling `loft` chords the wall); each leg's transition
is a `hexmesh.assemble.loft_fn` over the blend parameter rather than a `loft` of a section
...
evaluated at every node rather than `hexmesh.lift.from_grid`, which blends straight from
