...
# below) and meshed with ``LineMesh.curve``, so the wall's high-order nodes sit on
...
def _arc_mesh(p, n, element_tag):
    ...
    length.  ``LineMesh.curve`` evaluates ``p`` at every node -- corners *and* the
    private GLL interiors -- so no node lands on a chord."""
    return LineMesh.curve(p, LineMesh.arclength_fractions(p, n),
...
def fourier_ring(P, keep=FOURIER_KEEP):
    ...
    curve, so ``LineMesh.curve`` can place every GLL node *on* it instead of on the
...
def fourier_wall(P, order, element_tag):
    ...
    ``LineMesh.curve`` only ever makes an open chain, so weld the two ends into a loop
    the way the toolkit expects (``LineMesh.merge``)."""
    M = np.asarray(P).shape[0]
    return LineMesh.merge([LineMesh.curve(
