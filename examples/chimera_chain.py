"""``N_COPIES`` chimera units (see ``chimera.py``) chained end to end along ``z``,
alternating which of a unit's two manifolds -- pipe A or pipe B -- carries the
connection to the next unit.

Run with::

    PYTHONPATH=. python examples/chimera_chain.py

Produces ``chimera_chain.re2`` and ``chimera_chain.vtu``.
...
def build_junction(z0, run_minus, run_plus):
    ...
    return HexMesh.merge(blocks).translate((0.0, 0.0, z0))
