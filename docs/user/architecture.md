...
| `core/affine.py` | the affine maps behind the rung-preserving `translate` / `rotate` / `scale`: `translation` / `rotation` / `scaling` build a `(matrix, offset)` pair, `apply` maps any coordinate table whose trailing axis is the 3 components. Imports **no container** |
